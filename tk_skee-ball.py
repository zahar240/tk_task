...
scored = int(input("Введите количество очков: "))
price_gun = int(input("Введите стомость приза в билетах: "))

def buy_squirt_gun(score, price):
    move = ""
    if int(score / 12) >= price:
        move = 'Buy it!'
    else:
        move = 'Try again'
    return move

print(buy_squirt_gun(scored, price_gun))
